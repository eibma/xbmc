...
void CGameClient::InitSerialization(const CFileItem& file)
{
    ...
    else
    {
      ...
      if (gameBuffer)
      {
        ...
        if (length > MAX_SAVESTATE_CRC_LENGTH)
          m_saveState.SetGameCRCFromFile(m_gameFile.Path(), true);
        else
          m_saveState.SetGameCRCFromFile(reinterpret_cast<const char*>(gameBuffer), length);
      }
      else
        m_saveState.SetGameCRCFromFile(m_gameFile.Path());
