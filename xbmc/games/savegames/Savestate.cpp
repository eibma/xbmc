...
const CStdString &CSavestate::GetPath() const
{
  if (m_path.empty() && !m_gameClient.empty() && !m_gameCRC.empty())
  {
    CStdString hash;
    if (m_bAutoSave)
      hash = StringUtils::Format("%s/%s", m_gameClient.c_str(), m_gameCRC.c_str()); 
    else if (m_slot)
      hash = StringUtils::Format("%s/%s_%u", m_gameClient.c_str(), m_gameCRC.c_str(), m_slot);
    else
    {
      Crc32 crc;
      crc.Compute(m_label);
      hash = StringUtils::Format("%s/%s_%08x", m_gameClient.c_str(), m_gameCRC.c_str(), (unsigned __int32)crc);
    }
    hash += SAVESTATE_EXTENSION;
    m_path = URIUtils::AddFileToFolder(CProfilesManager::Get().GetSavegamesFolder(), hash);
  }
  return m_path;
}
...
void CSavestate::SetSaveTypeAuto()
{
  ...
  m_slot = 0;
  // Autosave on %s
  m_label = StringUtils::Format(g_localizeStrings.Get(15042).c_str(), CDateTime::GetCurrentDateTime().GetAsLocalizedDateTime().c_str());
}
...
void CSavestate::SetSaveTypeSlot(unsigned int slot)
{
  ...
  m_slot = slot;
  // Slot %d
   m_label = StringUtils::Format(g_localizeStrings.Get(15043).c_str(), slot);
}
...
void CSavestate::SetGameCRCFromFile(const CStdString &filename, bool forceFilename /* = false */)
{
  ...
  if (file.Open(filename) && (length = file.GetLength()) > 0)
  {
    if (length > MAX_SAVESTATE_CRC_LENGTH || forceFilename)
    {
      ...
      m_gameCRC = StringUtils::Format("%08x", (unsigned __int32)crc);
    }
    else
    {
