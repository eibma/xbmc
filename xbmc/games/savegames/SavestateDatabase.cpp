...
void CSavestateDatabase::CreateTables()
{
  try
  {
    BeginTransaction();
    CDynamicDatabase::CreateTables();
    CommitTransaction();
    return;
  }
...
CFileItem* CSavestateDatabase::CreateFileItem(const CVariant &object) const
{
  ...
  {
    CStdString strSlot = StringUtils::Format("%u", p.GetSlot());
...
void CSavestateDatabase::CreateAnalytics()
{
