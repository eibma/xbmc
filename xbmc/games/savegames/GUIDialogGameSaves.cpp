  ...
  case GUI_MSG_WINDOW_INIT:
    {
      ...
      if (g_application.m_pPlayer->IsPlayingGame())
      {
        ...
        if (wasFullscreen)
        {
          ...
          playSpeed = !g_application.m_pPlayer->IsPaused() ? g_application.m_pPlayer->GetPlaySpeed() : 0;
          if (playSpeed)
        ...
        boost::shared_ptr<CRetroPlayer> rp = boost::dynamic_pointer_cast<CRetroPlayer>(g_application.m_pPlayer->GetInternal());
...
void CGUIDialogGameSaves::Update()
{
  ...
  if (!m_gamePath.empty())
  {
    ...
    if (db.GetItemID("gamepath", m_gamePath, dbGamePathId))
    {
      ...
      m_vecItems->Sort(SortByDate, SortOrderDescending);
    }
  }
...
{
  if (playSpeed && g_application.m_pPlayer->IsPlayingGame())
  {
