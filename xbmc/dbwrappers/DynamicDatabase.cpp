...
{
  CStdString indexName = StringUtils::Format("idx_%s_link%d", MakeLinkTableName(primary, secondary).c_str(), ord); // idx_primarylinkseconday_link1
  return indexName;
}
...
void CDynamicDatabase::CreateTables()
{
  InitializeMainTable();
...
int CDynamicDatabase::AddObject(const ISerializable *obj, bool bUpdate /* = true */)
{
  ...
  try
  {
    ...
    if (!bExists)
    {
      ...
      {
        COLUMNS += ", id" + it->first;
        CStdString id = StringUtils::Format("%d", it->second);
        VALUES += ", " + id;
      }
...
bool CDynamicDatabase::DeleteObjectByID(int idObject, bool killOrphansMwahahaha /* = true */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
bool CDynamicDatabase::DeleteObjectByIndex(const std::string &column, const CVariant &value, bool deleteOrphans /* = true */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
