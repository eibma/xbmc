...
bool CDatabase::GetAllTables(CStdStringArray &tableList)
{
  ...
  try
  {
    ...
    else
    {
      strSQL = PrepareSQL(
        ...
        GetBaseDBName(), GetMinSchemaVersion()
      );
      if (m_pDS->query(strSQL.c_str()))
      {
        tableList.clear();
...
bool CDatabase::GetAllIndices(CStdStringArray &indexList)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
    ...
    else
    {
      ...
      for (std::vector<CStdString>::const_iterator it = tables.begin(); it != tables.end(); it++)
      {
        ...
        strSQL = PrepareSQL(
          ...
          GetBaseDBName(), GetMinSchemaVersion(), it->c_str()
        );
        if (!m_pDS->query(strSQL.c_str()))
          return false;
